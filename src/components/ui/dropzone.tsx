    ...
    onDropAccepted: async (newFiles) => {
      ...
      const onDropFilePromises = slicedNewFiles.map(async (file, index) => {
        if (fileCount + 1 > maxNewFiles) {
          const file = fileStatuses[index];
          if (file) await onRemoveFile(file.id);
        }
  ...
  ({ className, children, ...props }, forwardedRef) => {
    ...
    return (
      ...
      <div
        ...
        aria-label="dropzone"
        className={cn(
          "border-input ring-offset-background focus-visible:ring-ring flex items-center justify-center rounded-md border bg-transparent px-4 py-2 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none",
